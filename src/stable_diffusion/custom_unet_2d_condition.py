...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        Args:
            ...
            feature_extractor (`str`): use 'dino' or 'sd-dino' method to extract hidden layers of stable diffusion.
                'dift': Emergent Correspondence from Image Diffusion
                'sd-dino': A Tale of Two Features: Stable Diffusion Complements DINO for Zero-Shot Semantic Correspondence
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample, cached_hidden_states = upsample_block(
                    hidden_states=sample,
                    temb=emb,
            ...
            if feature_extractor == "dift":
                featmaps = featmaps + (sample,)
            elif feature_extractor == "sd-dino":
                featmaps = featmaps + cached_hidden_states
            else:
                raise ValueError(f"Invalid input: {feature_extractor}, 'dift' or 'sd-dino' should be used.")
        ...
        if not return_dict:
            return (sample, featmaps)

        return UNet2DConditionOutput(sample=sample, featmaps=featmaps)
